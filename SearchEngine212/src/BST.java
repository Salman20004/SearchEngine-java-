...
public class BST<T> {
	    ...
	    public boolean find(Relative rel){ //o(n) o(n)
			switch (rel) {
			   case Root:	// Easy case
				current = root;
				return true;
			   case Parent:
				if(current == root)
	                return false;
				current = findparent(current, root); //o(n) o(n)
				return true;
			   case LeftChild:
	    ...
	    private BSTNode<T> findparent(BSTNode<T> p, BSTNode<T> t) { //o(n) o(n)
			if(t == null)
	    ...
	    public boolean findkey(int tkey) { // O(logn)
	    ...
	    public boolean insert(int k, T val) { //logn
